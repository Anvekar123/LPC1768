...
#define TRIG_PIN (1 << 10)  // Define the pin for the trigger
#define ECHO_PIN (1 << 11) // Define the pin for the echo

 #define LED_TOGGLE (1<<26)							//0th bit is made high
 #define LED_ON() LPC_GPIO3->FIOSET = LED_TOGGLE	//Set P3.0 bit
 #define LED_OFF() LPC_GPIO3->FIOCLR =  LED_TOGGLE	//clear P3.0 bit


unsigned int delay(int);
unsigned long int temp1=0, temp2=0 ;
unsigned char Msg1[14] = {"Team 5"};
...
int main(void)
{
  ...
  if (distance < 10) {    
        ...
        LED_OFF();           // Turn off the LED if the distance is not less than 10 cm
    }
}
}
